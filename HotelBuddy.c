		...
		if (choice==1||match==1)
		{
			...
			while (match==0||choice==1)
			{
				...
				while (fread(&htemp,sizeof(htemp),1,fp1)==1&&match==0)
				{
					...
					{
						printf("Enter date on which you wish to book:/t(Format: DD/MM/YY)/n(Note:You can only schedule room booking only within 30 days from today )\n");fflush(stdin);
			...
			while(decision=='Y')
			{
				...
				switch (choice)
				{
					case 1:
						...
						break;
					case 2:
						printf("Update services:/t(Add hyphen before entering new service e.g(-----New service))\n");
						fflush(stdin);
						gets(services);fflush(stdin);
